...
# model_path = r"final_modelDCTOMP90.h5"
...
# Sidebar image
# st.sidebar.image("11.png", width=100, caption="")
# Sidebar links
...
st.image("User guide.png", width=400, caption="Flow Using CS-CNNVisionHub! 🚀", use_column_width=True)
...
with About_tab:
    ...
    with st.expander("Model Design & Implementation"):
        ...
        st.image("about.png", use_column_width=True, caption="Model Design & Implementation", output_format="JPEG", width=500)
...
with clas_tab:
    ...
    if methodCNN_choice == "DCT":
        model_path = r"final_modelDCTOMP70.h5"
        # Load the saved model
        loaded_model = load_model(model_path)
    ...
    if methodCNN_choice == "DWT":
        model_path = r"final_modelDWTOMP70.h5"
        # Load the saved model
        loaded_model = load_model(model_path)
        # st.image("graph.png", width=400, caption="Training & Validation Grafik", use_column_width=True)
        # Upload image through Streamlit
        uploaded_image = st.file_uploader("Choose an image...", type=["jpg", "jpeg", "png"])
    ...
    if methodCNN_choice == "FFT":
        model_path = r"final_modelFFTTOMP70.h5"
        # Load the saved model
        loaded_model = load_model(model_path)
        # Upload image through Streamlit
        uploaded_image = st.file_uploader("Choose an image...", type=["jpg", "jpeg", "png"])
...
with desc_tab:
    ...
    with st.expander("Adenocarcinoma"):
        ...
        col1.image("lungaca1.png", use_column_width=True, caption="Adenocarcinoma", output_format="JPEG", width=200)
        with st.container():
    ...
    with st.expander("Squamous Cell Carcinoma"):
        ...
        col2.image("lungscc1.png", use_column_width=True, caption="Squamous Cell Carcinoma", output_format="JPEG", width=200)
        with st.container():
            st.markdown("### Explanation")
    ...
    with st.expander("Large Cell Carcinoma"):
        ...
        col3.image("lungaca1.png", use_column_width=True, caption="Large Cell Carcinoma", output_format="JPEG", width=200)
        with st.container():
            st.markdown("### Explanation")
...
with cv_tab:
    ...
    st.image("1.png", width=400, caption="Andi Ainun Najib", use_column_width=True)
    st.markdown("<hr/>", unsafe_allow_html=True)
